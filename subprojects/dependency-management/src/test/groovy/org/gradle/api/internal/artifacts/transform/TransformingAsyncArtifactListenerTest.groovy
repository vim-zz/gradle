...
class TransformingAsyncArtifactListenerTest extends Specification {
    ...
    def invocation = Mock(Deferrable<TransformSubject>)
    def operationQueue = Mock(BuildOperationQueue)
    def listener = new TransformingAsyncArtifactListener([new BoundTransformStep(transformation, Stub(TransformUpstreamDependencies))], targetAttributes, [], result)
    ...
    def "runs cheap artifact transformations immediately when not scheduled"() {
        ...
        2 * invocation.getCompleted() >> Optional.of(Try.successful(TransformSubject.initial(artifact)))
        0 * operationQueue._
    }
}
